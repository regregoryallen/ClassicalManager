...
- Incremental scan via mtime/size change detection (V1 does full rescans; structure the scanner so this slots in).
- Additional export formats (PLS, XSPF) — added as new serializer implementations against the existing interface.
- Selection weighting / variety constraints (e.g. duration-weighting, "no two works by the same composer back-to-back").
...
- **Regeneration overwrites by title:** locate the existing playlist of the same name and replace its contents (clear-and-refill, or delete-and-recreate — pick the safer during implementation and document the choice).

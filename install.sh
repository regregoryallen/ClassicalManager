...
do_uninstall() {
    ...
    if [ "$is_system" = "1" ]; then
        ...
        sudo rm -f "$BIN_DIR/${APP_NAME}-rgtag"
        sudo rm -f "$DESKTOP_DIR/${APP_NAME}.desktop"
    else
        rm -rf "$found_dir"
        rm -f "$BIN_DIR/$APP_NAME"
        rm -f "$BIN_DIR/${APP_NAME}-rgtag"
...
install_cli_wrapper() {
    ...
    # Both scripts answer --help and --version before importing anything,
    # so these keep working even if the venv is later broken.
...
# Runs the app in its virtual environment, so no activation is needed.
# All arguments are passed through:  $APP_NAME --help
...
WRAPPER_EOF
    ...
    # The ReplayGain tagger is a separate program (it is the only part
    # that writes to audio files), so it gets its own wrapper rather than
    # a subcommand.
    local rgwrapper="$BIN_DIR/${APP_NAME}-rgtag"

    maybe_sudo tee "$rgwrapper" > /dev/null << RGWRAPPER_EOF
#!/usr/bin/env bash
# Classical Manager ReplayGain tagger wrapper — auto-generated by install.sh
# Dry-run is the default; nothing is written without --write.
# All arguments are passed through:  ${APP_NAME}-rgtag --help
exec "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/rgtag.py" "\$@"
RGWRAPPER_EOF

    maybe_sudo chmod 755 "$rgwrapper"
    success "ReplayGain tagger wrapper installed: $rgwrapper"
}
...
" 2>/dev/null || echo "$INSTALL_DIR/music_manager.db")
    ...
    echo -e "  ${BOLD}Tagger command:${RESET}    ${APP_NAME}-rgtag"
    ...
    echo "  ReplayGain tagger:    ${APP_NAME}-rgtag --help"

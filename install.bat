...
REM The ReplayGain tagger is a separate program - it is the only part that
REM writes to audio files - so it gets its own wrapper, not a subcommand.
REM Always console: it is interactive and reports as it goes.
> "!INSTALL_DIR!\!APP_NAME!-rgtag.bat" echo @echo off
>> "!INSTALL_DIR!\!APP_NAME!-rgtag.bat" echo REM Classical Manager ReplayGain tagger wrapper - auto-generated by install.bat
>> "!INSTALL_DIR!\!APP_NAME!-rgtag.bat" echo REM Dry-run is the default; nothing is written without --write.
>> "!INSTALL_DIR!\!APP_NAME!-rgtag.bat" echo "%%~dp0venv\Scripts\python.exe" "%%~dp0rgtag.py" %%*

echo   Tagger wrapper created: !INSTALL_DIR!\!APP_NAME!-rgtag.bat
...
echo   Tagger command:      !APP_NAME!-rgtag
